...
COMMON gdb COMMANDS:
  ...
  where/backtrace, frame, list (show call frames on stack; go to call frame;
                                list source code (and list more if typed again))
...
If you like a display more like the MARS simulator, type ^Xa
Type ^Xa again to return to normal mode.
...
====
Example gdb session to understand better the functions getargs() and getword()
for hw3:

[ NOTICE THAT gdb WILL REQUIRE YOU TO GIVE THE 'run' COMMAND. ]
MSPACMAN:hw3% gdb --args ./myshell
...
Reading symbols from /net/course/cs2600gc/hw3/myshell...done.
(gdb) where
No stack.
(gdb) next
The program is not being run.
(gdb) run
Starting program: /net/course/cs2600gc/hw3/myshell 
% exit
...
Reading symbols from /net/course/cs2600gc/hw3/myshell...done.
(gdb) break main
Breakpoint 1 at 0x80488a0: file hw3.c, line 101.
(gdb) run
Starting program: /net/course/cs2600gc/hw3/myshell 

Breakpoint 1, main (argc=1, argv=0xbffff8b4) at hw3.c:101
101	{
(gdb) list
...
getargs (cmd=0xbffff9b4 "\314\371\377\277\016", argcp=0xbffff9b0, 
    argv=0xbffff960) at hw3.c:55
55          char *cmdp = cmd;
(gdb) where
#0  getargs (cmd=0xbffff9b4 "\314\371\377\277\016", argcp=0xbffff9b0, 
    argv=0xbffff960) at hw3.c:55
#1  0x080488e5 in main (argc=1, argv=0xbffffb34) at hw3.c:109
(gdb) list
...
(gdb) where
#0  getword (begin=0xbffff9b4 "ls README\n", end_ptr=0xbffff928) at hw3.c:35
#1  0x080487df in getargs (cmd=0xbffff9b4 "ls README\n", argcp=0xbffff9b0, 
    argv=0xbffff960) at hw3.c:66
#2  0x080488e5 in main (argc=1, argv=0xbffffb34) at hw3.c:109
(gdb) list
...
42                  perror("getenv");
(gdb) frame 0
#0  getword (begin=0xbffff9b4 "ls", end_ptr=0xbffff928) at hw3.c:38
38          *end_ptr = end;
(gdb) list
33          while ( *end != '\0' && *end != '\n' && *end != ' ' )
34              end++;  /* Keep going. */
...
51       */
(gdb) next
getargs (cmd=0xbffff9b4 "ls", argcp=0xbffff9b0, argv=0xbffff960) at hw3.c:68
68              argv[i++] = cmdp;
(gdb) where
...
$30 = 2
(gdb) next
main (argc=1, argv=0xbffffb34) at hw3.c:112
112             if ( childargc > 0 && strcmp(childargv[0], "exit") == 0 )
(gdb) list
107             printf("%% "); /* printf uses %d, %s, %x, etc.  See 'man 3 printf' */
108             fflush(stdout); /* flush from output buffer to terminal itself */
...
$36 = 2
(gdb) frame 0
#0  main (argc=1, argv=0xbffffb34) at hw3.c:117
117                 execute(childargc, childargv);
(gdb) list
112             if ( childargc > 0 && strcmp(childargv[0], "exit") == 0 )
113                 exit(0);
...
120     }
(gdb) step
execute (argc=2, argv=0xbffff960) at hw3.c:80
80          childpid = fork();
(gdb) list
75
76      static void execute(int argc, char *argv[])
